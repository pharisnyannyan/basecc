...
TEST(check_typedef_const_cast, "check typedef const cast") {
  ...
  checker_init(&checker, "typedef const int *ConstIntPtr;"
...
TEST(check_extern_function_declaration, "check extern function declaration") {
  ...
  checker_init(&checker, "extern int write(int fd, const char *buf, int count);"
...
TEST(check_parenthesized_arithmetic, "check parenthesized arithmetic") {
  ...
  checker_init(&checker, "int main(){return (1 + 2) * (3 - 4) / 5 + 6;}");
...
TEST(check_sizeof, "check sizeof") {
  ...
  checker_init(&checker,
               ...
               "int main(){struct S s; int *p; "
               "return sizeof(s) + sizeof(*p) + sizeof(int) + sizeof(char);}");
